...
export default function CheckoutPage() {
  ...
  async function placeOrder(event: FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    try {
      const response = await fetch("/api/orders", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customer: { name: form.get("name"), email: form.get("email"), phone: form.get("phone") }, notes: form.get("notes"), items: items.map(item => ({ id: item.productId || item.id, variation: item.variation, quantity: item.quantity })), couponCode, paymentMethod: payment, senderNumber: form.get("senderNumber"), transactionId: form.get("transactionId"), idempotencyKey }) });
  ...
  return <main><MainHeader />
    ...
    <section className="section checkout-section"><form className="shell checkout-layout" onSubmit={placeOrder}>
      <div className="checkout-main">
        ...
        <section className="checkout-card"><div className="checkout-card-heading"><span>02</span><div><h2>Payment method</h2><p>Select how you would like to complete your payment.</p></div></div>
