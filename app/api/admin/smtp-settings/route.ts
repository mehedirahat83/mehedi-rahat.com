...
export async function PUT(request: Request) {
  ...
  const current = await getSmtpConfig(); const host = text(body?.host, 255); const username = text(body?.username, 255); const password = text(body?.password, 500) || current.password; const fromEmail = text(body?.fromEmail, 255); const fromName = text(body?.fromName, 120); const notificationEmail = text(body?.notificationEmail, 255); const port = Number(body?.port); const secure = body?.secure === true;
  if (!host || !username || !password || !fromEmail || !fromName || !notificationEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(notificationEmail) || !Number.isInteger(port) || port < 1 || port > 65535) return Response.json({ ok: false, error: "Enter valid SMTP, sender and support notification email details." }, { status: 400 });
  await saveSmtpSettings({ host, port, secure, username, password, fromEmail, fromName, notificationEmail });
